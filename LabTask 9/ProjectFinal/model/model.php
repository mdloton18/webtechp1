...
class db
{
	...
	{
		$stmt = $conn -> prepare("INSERT INTO $table (eventname, totalperson, eventphone, eventdate, message) VALUES (?, ?, ?, ?, ?)");
	...
	function SearchFood($conn,$table,$food_name)
	{
		$stmt = $conn -> query("SELECT * FROM $table WHERE food_name LIKE '%".$search."%'");
		return $stmt;
	}
